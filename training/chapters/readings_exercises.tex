...
\section{Scientific Claim Boundaries}
The safest writing pattern is to name the evidence type before making the claim. Students should practice replacing broad claims with source-backed claims that state uncertainty directly.

\subsection{Tentative Chemical Match}
\SafeClaim
{This sample contains methyl salicylate.}
{A tentative GC-MS annotation matched methyl salicylate; confirmation would require standards or other confirmatory evidence.}
{What evidence distinguishes a library match from confirmed identity?}

\subsection{Database Presence}
\SafeClaim
{The chemical occurs in this plant because it appears in a public database.}
{Public database records report occurrence or annotation evidence for this chemical in specified sources; the current sample requires its own evidence.}
{Is the database record about the same organism, sample, and analytical method?}

\subsection{Source Annotation as Variable}
\SafeClaim
{This full database sentence proves the compound is anti-inflammatory.}
{The source annotation contains biomedical language that can be recorded as evidence; an analysis-ready trait requires a documented extraction rule and caveat.}
{Which exact field was extracted, and what values are allowed in the variable?}

\subsection{Pathway Context}
\SafeClaim
{The pathway is active in the sample.}
{KEGG provides pathway or reaction context for the compound; pathway activity was not measured in this project.}
{Was pathway flux, expression, enzyme activity, or metabolite abundance directly tested?}

\subsection{Hazard Screening}
\SafeClaim
{The sample is dangerous.}
{One or more chemicals have hazard annotations that justify screening or follow-up; risk assessment requires exposure, concentration, route, organism, and context.}
{What dose or exposure information is available?}

\subsection{Natural Product Occurrence}
\SafeClaim
{LOTUS proves this compound is in the student's organism.}
{LOTUS-derived occurrence or taxonomy evidence supports natural product context; occurrence in the student's organism requires matching source evidence or direct measurement.}
{Does the source record name the same organism or only a related taxonomic group?}

\subsection{Bioactivity and Mechanism}
\SafeClaim
{The compound has a therapeutic mechanism in this system.}
{Bioactivity, target, potency, or pharmacology annotations can support hypothesis generation; mechanism or clinical relevance requires direct experimental evidence.}
{What assay system, target, potency, and biological context produced the annotation?}

\subsection{Sparse Trait Matrices}
\SafeClaim
{The heatmap shows clear chemical groups.}
{The trait matrix suggests exploratory grouping under the selected profile and confidence threshold; sparse columns and source coverage must be checked before stronger interpretation.}
{Do the groups persist after low-confidence or sparse variables are removed?}
