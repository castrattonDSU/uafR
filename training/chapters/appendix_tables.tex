...
\begin{programbox}{Validation Naming Note}
Inside a saved \texttt{categorate()} result, the compact validation table is named \texttt{ValidationSummary}. A direct call to \texttt{validateCategorateResult(result)} returns a list whose corresponding validation table is named \texttt{Summary}, along with \texttt{TableQuality}, \texttt{SourceDiagnostics}, \texttt{Issues}, and \texttt{DataDictionary}. Both names are intentional; students should record which object they inspected.
\end{programbox}
...
\path{10_species_phytochemistry.R} & Runs a simulated, offline species-first phytochemistry extension and optional live PubMed discovery.\\
\path{run_training_smoke_tests.R} & Runs lightweight checks across the training scripts.\\
\bottomrule
\end{tabularx}

\section{Student Bundle Files}
...
\path{uafR_QUICK_REFERENCE.md} & Gives students a compact command reference for setup, offline workflow, live smoke test, updates, and common problems.\\
\path{CHECKSUMS.csv} & Records generated bundle file sizes and MD5 checksums for copying and distribution checks.\\
\path{verify_bundle_integrity.R} & Confirms copied or unzipped bundle files still match \path{CHECKSUMS.csv}.\\
...
validation <- validateCategorateResult(smoke)
validation$Summary
smoke$SourceCoverage
smoke$ChemicalTraitReport
\end{rcode}

Run this optional live test only after the offline acceptance workflow passes.
The expected product is a research-grade result object with an explicit
validation summary, source-coverage rows for both query chemicals, and a
compact trait report. A service warning or no-hit result should be recorded in
the query log; it should not be replaced with an inferred value.

\begin{checklistbox}{Before Scaling Beyond the Smoke Test}
\begin{itemize}
  \item Confirm that the two query names are the intended chemical identities.
  \item Inspect validation and source diagnostics before interpreting traits.
  \item Save the result object, query settings, package version, and session
        information.
  \item Keep caching enabled and increase query size gradually.
  \item Pause after repeated service-busy responses and resume from the same
        cache rather than restarting immediately.
\end{itemize}
\end{checklistbox}
