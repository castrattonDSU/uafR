...
categorate result, or a \`ChemicalTraits\` table into a wide matrix
whose columns are controlled ontology keys. This gives researchers a
more stable analysis surface than source-specific trait names while
...
- mode:
  ...
  stores ontology-row counts, and \`"confidence"\` stores the maximum
  confidence score for each compound-ontology pair.
...
A data frame with \`Query\`, \`CID\`, and one column per selected
ontology key.
